...
Item {
    ...
    Rectangle {
        ...
        // Idle-inhibited badge — a small "hand" overlay when something is keeping the
        // machine awake, so the state is visible without opening the panel.
        NIcon {
            visible: (main?.available ?? false) && (main?.inhibited ?? false)
            anchors.right: parent.right
            anchors.top: parent.top
            anchors.margins: 1
            icon: "hand"
            pointSize: Style.toOdd(root.iconSize * 0.30)
            applyUiScale: false
            color: Color.mError
        }
    }
    ...
    function barTooltip() {
        ...
        if (main.batteryPresent) {
            ...
            if (main.batteryTimeToEmpty >= 0) {
                ...
                head = s + pct + " — " + h + "h " + m + "m remaining"
            } else if (main.batteryTimeToFull >= 0) {
                var h2 = Math.floor(main.batteryTimeToFull / 60)
                var m2 = main.batteryTimeToFull % 60
                head = s + pct + " — " + h2 + "h " + m2 + "m to full"
            } else {
                head = s + pct
            }
        } else {
            var ps = main.powerSource === "ac"
                ? (pluginApi?.tr("panel.power_ac") ?? "AC power")
                : main.powerSource === "battery"
                    ? (pluginApi?.tr("panel.power_battery") ?? "On battery")
                    : main.powerSource
            head = main.profile + (ps ? " — " + ps : "")
        }

        if (main.inhibited) head += "\n" + (pluginApi?.tr("panel.inhibited") ?? "Idle inhibited")
        return head
    }
